...
public class startingTree extends Tree implements StateNodeInitialiser {
     ...
     */
    public int[] get_matching_taxa (int nMatches, int iTaxon){
        //match collector
        ...
        if(sequencesAreClustered){
            // given that taxa with matching sequences are clustered together
     ...
     */
    public Node get_cluster_tree(int iTaxon, double scarringStop, Alignment taxa, int iCluster){
        ...
        for (int iMatch=0; iMatch < nMatches; iMatch++){
            ...
            parent.setHeight(divTimes * (iMatch+1) * Randomizer.nextDouble());
     ...
     */
    public int update_iTaxon(int iTaxon){
        int nMatches;
        if (sequencesAreClustered) {
