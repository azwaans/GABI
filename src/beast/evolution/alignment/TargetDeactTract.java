...
public class TargetDeactTract extends BEASTObject {
    ...
    public List<TargetStatus> getContainedStatuses(){
        ...
        int[][] allPossibleStat = new int[(int) (java.lang.Math.pow(2,M))][M];
        for (int i = 0; i < M;i++ )  {
            for( int j = 0; j <= (int) (java.lang.Math.pow(2,i+1)-1); j++ ) {
                int startIndex = (int) ((int)(j)*(Math.pow(2,M-i-1)));
                int endIndex = (int) ((j+1)*(java.lang.Math.pow(2,M-i-1))-1);
