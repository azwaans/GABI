...
//    AncState is a list of indel states, WC or SGWC

    final public Input<List<IndelSet>> dataInput = new Input<>("value",
            "List of indel sets, as Wild Cards or Singleton Wild Cards", Input.Validate.REQUIRED);
    ...
    public static AncStates intersect(AncStates ancStates1, AncStates ancStates2) {
        ...
        /*From GAPML's code, only a anc1 check for null. should we also check anc2???. If on of the sets id empty,
        , the intersection is empty */


        if(ancStates1.indelSetList.size() == 0) {
    ...
    public boolean isPossible(List<IndelSet.TargetTract> state) {
        ...
        //shortcut: if both ancestral states are of different size, already know the
        //INTERSECT SUPPOSED TO BE ZERO
